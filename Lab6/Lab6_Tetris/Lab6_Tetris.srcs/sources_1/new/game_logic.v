...
module game_logic(
        ...
        input start_sig,                        // 开始信号
        input[15:0] enable_moving,              // 正在下落方块形状
        input verticalspeed_out,                // 垂直下落信号输入
        output[10:0] h,                         // 下落小方块水平方向位置
        output[10:0] v,                         // 下落小方块垂直方向位置
        output[499:0] enable_little,            // 游戏界面存储寄存器
        output loading_square,                  // 请求加载新的小方块
        output[8:0] little_square_num,          // 正在下落方块坐标
        output over_out,                        // 输出游戏结束信号
        output[15:0] score_out,                 // 输出计分结果
        output level_up                         // 输出提高难度级别信号
    );

    reg inc10_n;                                // 能否右移，为 0 则可以
    reg[499:0] enable_little_r;                 // 每一块小方块是否显示，为1则显示
    reg[8:0] little_square_num_r;               // 正在下落方块坐标
    reg[10:0] ish;                              // 下落小方块水平方向位置
    reg[10:0] isv;                              // 下落小方块垂直方向位置
    reg dec10_n;                                // 能否左移，为 0 则可以
    wire[19:0] sub_line;                        // 共20行，每行的消除信号
    wire sub_line_total;                        // 消除行信号，为1则有行要消除
    wire temp;
    wire v10_enable;
    reg loading_square_r;                       // 加载新的小方块
    ...
    always @ ( posedge clk or negedge rst_n ) begin
        if( !rst_n )
            little_square_num_r	<=	9'd10;                          // 小方块初始位置 10
        else if( loading_square_r )                                 // 随机刷新位置
            little_square_num_r	<=	rand_num % 8 + 3'd5;

        else if( move_right == 1'b1 )                               // 向右移动
        begin
            if( !inc10_n )
                little_square_num_r	<=	little_square_num_r + 1'b1;
        end
        else if( move_left == 1'b1 )	                            // 向左移动
        begin
            if( !dec10_n )
                little_square_num_r	<=	little_square_num_r - 1'b1;
        end
        else if( v10_enable )                                       // 小方块下落
    ...
    always @ ( posedge clk or negedge rst_n ) begin
        if( !rst_n )
            ...
            ish	<=	11'd54+(rand_num%8)*20;
        else if( move_right == 1'b1 ) begin
            if( !inc10_n )
                ish	<=	ish + 11'd20;
            else
                ish	<=	ish;
    ...
    always@(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            if((score[3:0]+sub_line[0]+sub_line[1]+sub_line[2]+sub_line[3]+sub_line[4]+
                    ...
                    sub_line[15]+sub_line[16]+sub_line[17]+sub_line[18]+sub_line[19])>9) begin
