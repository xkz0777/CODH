...
module display_next_square(
    ...
    reg[15:0] enable_display_r;                         // 下一个小方块存储寄存器 4x4
    ...
    always@(posedge clk or negedge rst_n) begin
        if(!rst_n)
            ...
            case(square_type)
                ...
                3'b001:
                    enable_display_r <= 16'b0000_0110_0110_0000;
                3'b010:
                    enable_display_r <= 16'b0010_0010_0010_0010;
                3'b011:
                    enable_display_r <= 16'b0000_0011_0110_0000;
                3'b100:
                    enable_display_r <= 16'b0000_0110_0011_0000;
                3'b101:
                    enable_display_r <= 16'b0000_0011_0010_0010;
                3'b110:
                    enable_display_r <= 16'b0000_0011_0001_0001;
                default:
                    enable_display_r <= 16'b0000_0110_0110_0000;
            endcase
        end
    ...
    generate
        genvar i1,j1;
        for(i1=0;i1<4;i1=i1+1) begin:fwgwgwr
            for(j1=0;j1<4;j1=j1+1) begin:fewrgnfq
                always@(posedge clk or negedge rst_n) begin
                    if(!rst_n)
                        enable_next_display_h[i1*4+j1] <= 1'b0;
                    else if(enable_display_r[i1*4+j1] == 1'b1) begin
                        if(col_addr_sig == 11'd451+j1*20)
                            enable_next_display_h[i1*4+j1] <= 1'b1;
                        else if(col_addr_sig == 11'd470+j1*20 )
                            enable_next_display_h[i1*4+j1] <= 1'b0;
                        else
    ...
    generate
        genvar i2,j2;
        for(i2=0;i2<4;i2=i2+1) begin:gwhghw
            for(j2=0;j2<4;j2=j2+1) begin:whrwge
                always@(posedge clk or negedge rst_n) begin
                    if(!rst_n)
                        enable_next_display_v[i2*4+j2] <= 1'b0;
                    else if(enable_display_r[i2*4+j2] == 1'b1) begin
