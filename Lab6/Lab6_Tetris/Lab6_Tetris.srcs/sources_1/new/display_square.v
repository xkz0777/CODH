...
module display_square(
    ...
    always@(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            enable_blue_little_h[499:400] <= 100'd0;
    ...
    generate
        genvar i,j;
        for(i=0;i<20;i=i+1) begin:haha

            for(j=0;j<10;j=j+1) begin :hehe
                always@(posedge clk or negedge rst_n) begin
                    if(!rst_n)
                        enable_blue_little_h[5+i*20+j] <= 1'b0;
                    else if(enable_little[5+i*20+j] == 1'b1) begin
                        if(col_addr_sig == 11'd51+j*20)
                            enable_blue_little_h[5+i*20+j] <= 1'b1;
                        else if(col_addr_sig == 11'd70+j*20)
                            enable_blue_little_h[5+i*20+j] <= 1'b0;
                        else
    ...
    generate
        genvar ii,jj;
        for(ii=0;ii<20;ii=ii+1) begin:gregher

            for(jj=0;jj<10;jj=jj+1) begin :hwrgw
                always@(posedge clk or negedge rst_n) begin
                    if(!rst_n)
                        enable_blue_little_v[5+ii*20+jj] <= 1'b0;
                    else if(enable_little[5+ii*20+jj] == 1'b1) begin
    ...
    generate
        genvar k,h;
        for(k=0;k<20;k=k+1) begin :shs
            for(h=0;h<10;h=h+1) begin:fewjg
